...
class BoxCoder(nn.Module):
	...
	def forward(self, boxes):#256,21,7,2
		self.bound = self.decode(boxes) # (bs, patch_count, channel, 2)
		points = self.meshgrid(self.bound)#1,12,307,12,1,2
	...
	def meshgrid(self, boxes): # Input: pred_boxes. To get the sampling location
		...
		channel_boxes = torch.zeros((boxes.shape[0], boxes.shape[1], 2)).to(self.device)#1,12,2
		channel_boxes[:, :, 1] = 1.0
...
class OffsetPredictor(nn.Module):
	...
	def forward(self, X): # Input: (bs, channel, seq_len)
		...
		patch_X = F.unfold(patch_X, kernel_size=(self.patch_size, self.channel), stride=self.stride).permute(0, 2, 1) #256,7,352# (B, patch_count, patch_size*channel)
		#1,12,3684
		# decoupling
		B, patch_count = patch_X.shape[0], patch_X.shape[1] #256  21
		patch_X = patch_X.contiguous().view(B, patch_count, self.patch_size, self.channel)#256,21,32,7 #1,12,12,307
		patch_X = patch_X.permute(0, 1, 3, 2)#256,21,7,32

		# patch_X: (B, patch_count, channel, patchsize)
		patch_X = patch_X.contiguous().view(B*patch_count*self.channel, 1, self.patch_size)#3684,1,12

		# calculate the bias throughout 2 Conv1d
		pred_offset = self.offset_predictor(patch_X)#684,2,1
		pred_offset = pred_offset.view(B, patch_count, self.channel, 2).contiguous()#1,12,307,2
...
class DepatchSampling(nn.Module):
	...
	def get_sampling_location(self, X): # Input: (bs, channel, window)
		...
		"""
		# get offset
		pred_offset = self.offset_predictor(X)#1,12,307,2
